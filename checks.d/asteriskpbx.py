...
# SIP Peers
        ...
        monitored_peers = re.findall(
            r'Monitored: ([0-9]+) online, ([0-9]+) offline', siptotals)[0]
        unmonitored_peers = re.findall(
            r'Unmonitored: ([0-9]+) online, ([0-9]+) offline', siptotals)[0]
...
# SIP Trunks (You have to add '-trunk' string into your SIP trunk name to detect it as a Trunk)
        ...
        trunks = re.finditer(
            '^.*-trunk.*([OK|UN].*)', sip_result.data, re.MULTILINE)
...
# IAX2 Peers
        ...
        iax_peers_unmonitored = re.findall(
            r'([0-9]+) unmonitored', iax_total_line)[0]
...
# G729 Codecs
        ...
        if 'No such command' not in g729_total_line:
            g729_total = re.findall(r'([0-9]+) licensed', g729_total_line)[0]
            g729_encoders = re.split('/', g729_total_line)[0]
            g729_decoders = re.findall(
                r'([0-9]+) encoders/decoders', g729_total_line)[0]
...
# Asterisk Uptime
        ...
        if "day" in system_total_line:
            system_uptime_days = re.findall(
                r'([0-9]+) day', system_total_line)[0]
        if "hour" in system_total_line:
            system_uptime_hours = re.findall(
                r'([0-9]+) hour', system_total_line)[0]
        if "minute" in system_total_line:
            system_uptime_minutes = re.findall(
                r'([0-9]+) minute', system_total_line)[0]
        if "second" in system_total_line:
            system_uptime_seconds = re.findall(
                r'([0-9]+) second', system_total_line)[0]

        system_uptime = (int(system_uptime_days) * 86400) + (int(system_uptime_hours)
                                                             * 3600) + (int(system_uptime_minutes) * 60) + int(system_uptime_seconds)
        ...
        if "day" in asterisk_total_line:
            asterisk_last_reload_days = re.findall(
                r'([0-9]+) day', asterisk_total_line)[0]
        if "hour" in asterisk_total_line:
            asterisk_last_reload_hours = re.findall(
                r'([0-9]+) hour', asterisk_total_line)[0]
        if "minute" in asterisk_total_line:
            asterisk_last_reload_minutes = re.findall(
                r'([0-9]+) minute', asterisk_total_line)[0]
        if "second" in asterisk_total_line:
            asterisk_last_reload_seconds = re.findall(
                r' ([0-9]+) second', asterisk_total_line)[0]

        asterisk_last_reload = (int(asterisk_last_reload_days) * 86400) + (int(asterisk_last_reload_hours)
                                                                           * 3600) + (int(asterisk_last_reload_minutes) * 60) + int(asterisk_last_reload_seconds)
...
# MFCR2 Channels
        ...
        self.gauge('asterisk.mfcr2.available.channels',
                   mfcr2_available_channels)
...
# SCCP Devices
        ...
        if "No such command" not in sccp_result.data:
            ...
            sccp_devices = re.finditer(
                '^.*.SEP.*', sccp_result.data, re.MULTILINE)
